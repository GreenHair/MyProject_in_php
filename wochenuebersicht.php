...
$befehl = "select laden.name, sum(betrag) as summe from ausgaben join rechnung on rechnung.id = ausgaben.rechnungsnr join laden on rechnung.laden = laden.ID where week(rechnung.datum,1) = week(current_date,1) and rechnung.einmalig = 1 group by laden.name order by summe desc;";
$result = $db->query($befehl);
